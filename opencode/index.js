...
export const OpenPlanAnnotatorPlugin = async (ctx) => {
    ...
    const implementationAgent = implementationHandoff.enabled
        ? implementationHandoff.agent
        : undefined;
    ...
    async function shouldInjectPlanReviewInstructions(sessionID) {
        ...
        try {
            ...
            const agent = agents.find(
                (candidate) => candidate.name === currentAgent
            );
    ...
    async function handoffToImplementationAgent(sessionID) {
        ...
        if (!sessionID) {
            return {
                ...
                warning:
                    "Could not auto-switch because the current session ID was unavailable.",
            };
        }
        ...
        try {
            ...
            return {
                agent: implementationAgent,
                warning: `Could not auto-switch to \`${implementationAgent}\`: ${getErrorMessage(
                    error
                )}`,
            };
        }
    ...
    return {
        "experimental.chat.system.transform": async (input, output) => {
            ...
            if (
                currentSystem.includes("title generator") ||
                currentSystem.includes("generate a concise title")
            ) {
                return;
            }

            if (!currentSystem.includes("submit_plan")) {
                const shouldInject = await shouldInjectPlanReviewInstructions(
                    input?.sessionID
                );
                if (shouldInject) {
            ...
            submit_plan: tool({
                ...
                args: {
                    ...
                    summary: tool.schema
                        .string()
                        .optional()
                        .describe("Optional one-line plan summary"),
                },
                ...
                async execute(args, context) {
                    ...
                    if (result.approved) {
                        ...
                        const handoffResult =
                            await handoffToImplementationAgent(
                                context.sessionID
                            );
                        if (handoffResult) {
                            if (handoffResult.warning) {
                                lines.push(
                                    `Auto-switch warning: ${handoffResult.warning}`
                                );
                            } else {
                                lines.push(
                                    `Auto-switched to the \`${handoffResult.agent}\` agent for implementation.`
                                );
                            }
                        }
