...
export async function resolveImplementationHandoff(directory) {
  ...
  const globalConfigPath = globalConfigDir
    ? join(globalConfigDir, "opencode", CONFIG_FILE_NAME)
    : undefined;
  const projectConfigPath = directory
    ? join(directory, ".opencode", CONFIG_FILE_NAME)
    : undefined;

  const globalConfig = globalConfigPath
    ? parseImplementationHandoff(await readConfigFile(globalConfigPath))
    : {};
  const projectConfig = projectConfigPath
    ? parseImplementationHandoff(await readConfigFile(projectConfigPath))
    : {};

  const enabled =
    projectConfig.enabled ?? globalConfig.enabled ?? DEFAULT_HANDOFF_ENABLED;
  const agent =
    projectConfig.agent ?? globalConfig.agent ?? DEFAULT_IMPLEMENTATION_AGENT;
